...
import { queuePrompt, getImageUrl, getComfyUrl, stopAll, uploadImage } from "../api/comfyApi";
import { addGenerationToLibrary } from "../store/mediaStore";
import MediaHistory from "./MediaHistory";
import { addToHistory } from "../utils/historyLimit";
import { log } from "../store/logStore";
import { buildOmniVoiceCloneWorkflow } from "../workflows/omnivoiceClone";

function OmniVoiceCloneNode({ id, data, selected }: NodeProps) {
  ...
  const connectedRefAudioUrl = useMemo(() => {
    const edge = edgesAll.find((e) => e.target === id && e.targetHandle === "ref_audio");
    if (!edge) return null;
    const srcNode = nodesAll.find((n) => n.id === edge.source);
    if (!srcNode) return null;
    const sd = srcNode.data as any;
    return sd.widgetValues?._previewUrl || sd.widgetValues?._audioUrl || null;
  ...
  const handleGenerate = useCallback(async (e: React.MouseEvent) => {
    ...
    if (!connectedRefAudioUrl) { setError("Connect a reference audio (TTS / Music / Import)"); setProcessing(false); return; }
    ...
    const refText = wv.refText || "";
    const modelPath = wv.modelPath || "omnivoice";
    const precision: "fp16" | "bf16" | "fp32" = wv.precision || "fp16";
    const loadAsr = wv.loadAsr ?? true;

    log("OmniVoice Clone rendering", { nodeId: id, nodeType: "fs:omnivoiceClone", nodeLabel: "OmniVoice Clone" });

    try {
      // Upload reference audio to ComfyUI input/ folder, then LoadAudio reads it.
      const ext = (() => {
        if (connectedRefAudioUrl.startsWith("data:audio/wav")) return "wav";
        if (connectedRefAudioUrl.startsWith("data:audio/mp3") || connectedRefAudioUrl.startsWith("data:audio/mpeg")) return "mp3";
        if (connectedRefAudioUrl.startsWith("data:audio/ogg")) return "ogg";
        if (connectedRefAudioUrl.startsWith("data:audio/flac")) return "flac";
        const m = connectedRefAudioUrl.match(/\.([a-zA-Z0-9]{3,4})(?:\?|$)/);
        return m ? m[1].toLowerCase() : "wav";
      })();
      const refFileName = `fs_omni_ref_${id}_${Date.now()}.${ext}`;
      let uploadedName: string;
      if (connectedRefAudioUrl.startsWith("data:")) {
        uploadedName = await uploadImage(connectedRefAudioUrl, refFileName);
      } else {
        const resp = await fetch(connectedRefAudioUrl);
        ...
        uploadedName = await uploadImage(dataUrl, refFileName);
      }

      const workflow = buildOmniVoiceCloneWorkflow({
        ...
        refAudioFileName: uploadedName, refText,
      });
          ...
          if (history[promptId]) {
            ...
            for (const nId of Object.keys(outputs || {})) {
              ...
              if (media && media.length > 0) {
                ...
                log("OmniVoice Clone complete", { nodeId: id, nodeType: "fs:omnivoiceClone", nodeLabel: "OmniVoice Clone", status: "success", details: `${numStep}st cfg${guidanceScale}` });
                addGenerationToLibrary(apiUrl, {
                  prompt: promptText, model: "OmniVoice Clone (k2-fsa)", seed: String(seed),
                  steps: numStep, cfg: guidanceScale, width: 0, height: 0, nodeType: "fs:omnivoiceClone",
            ...
            if (st?.completed || st?.status_str === "error") {
              ...
              log("OmniVoice Clone failed", { nodeId: id, nodeType: "fs:omnivoiceClone", nodeLabel: "OmniVoice Clone", status: "error" });
              setProcessing(false);
              return;
            }
          }
      ...
      log("OmniVoice Clone error", { nodeId: id, nodeType: "fs:omnivoiceClone", nodeLabel: "OmniVoice Clone", status: "error", details: err.message });
      setProcessing(false);
    }
  }, [id, edgesAll, nodesAll, nodeData.widgetValues, updateWidgetValue, connectedRefAudioUrl]);

  const textHL = connectingDir === "source" && (connectingType === "TEXT" || connectingType === "*") ? "highlight" : "";
  const refHL = connectingDir === "source" && (connectingType === "AUDIO" || connectingType === "*") ? "highlight" : "";
  const outputHL = connectingDir === "target" && (connectingType === "AUDIO" || connectingType === "*") ? "highlight" : "";
  const hasCompatible = connectingType ? !!(textHL || refHL || outputHL) : false;
  ...
  return (
    <div className={`mmaudio-node omnivoice-node ${selected ? "selected" : ""} ${dimClass}`} onClick={() => setSelectedNode(id)}>
      <div className="mmaudio-node-inner">
        <div className="mmaudio-accent" style={{ background: "#d96cb1" }} />
        <div className="mmaudio-header">
          <span className="mmaudio-icon">🎤</span>
          <div className="mmaudio-header-text">
            <span className="mmaudio-title">OmniVoice Clone</span>
            <span className="mmaudio-status">{processing ? "CLONING..." : `${language} · ${numStep}st`}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mmaudio-inputs">
        ...
        <div className="nanob-input-row">
          <Handle type="target" position={Position.Left} id="ref_audio" className={`slot-handle ${refHL}`} style={{ color: "#ec4899" }} />
          <TypeBadge color="#ec4899">AUD</TypeBadge>
          <span className="nanob-input-label">Reference Voice</span>
        </div>
      </div>
...
export default memo(OmniVoiceCloneNode);
