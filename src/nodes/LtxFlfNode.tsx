...
import { buildLtxFlfWorkflow } from "../workflows/ltxFlf";

// 0.5s 16kHz mono PCM — ComfyUI validates LoadAudio's combo against files in
// input/ even when ComfySwitch routes the audio chain away.
function makeSilentWavDataUrl(): string {
  const sampleRate = 16000;
  const numSamples = Math.floor(0.5 * sampleRate);
...
function LtxFlfNode({ id, data, selected }: NodeProps) {
  ...
  const handleGenerate = useCallback(async (e: React.MouseEvent) => {
    ...
    log("LTX FLF2V rendering", { nodeId: id, nodeType: "fs:ltxFlf", nodeLabel: "LTX 2.3 FLF2V" });
    ...
    try {
      ...
      const ts = Date.now();
      const firstName = await uploadImage(await fetchToDataUrl(firstFrameUrl), `fs_ltxflf_ff_${id}_${ts}.png`);
      const lastName  = await uploadImage(await fetchToDataUrl(lastFrameUrl),  `fs_ltxflf_lf_${id}_${ts}.png`);
      const silentAudioFile = await uploadImage(makeSilentWavDataUrl(), "fs_ltxflf_silent.wav");

      const workflow = buildLtxFlfWorkflow({
        prompt: promptText,
          ...
          if (history[promptId]) {
            ...
            for (const nId of Object.keys(outputs || {})) {
              ...
              if (media && media.length > 0) {
                ...
                log("LTX FLF2V complete", { nodeId: id, nodeType: "fs:ltxFlf", nodeLabel: "LTX 2.3 FLF2V", status: "success", details: `${frames}f ${width}x${height}` });
                addGenerationToLibrary(apiUrl, {
                  prompt: promptText, model: "LTX 2.3 22B FLF2V", seed: String(seed),
                  steps, cfg, width, height, nodeType: "fs:ltxFlf",
            ...
            if (st?.completed || st?.status_str === "error") {
              ...
              const execInterrupted = messages.find((m: any) => m[0] === "execution_interrupted");
              const executedOutputs = Object.keys(history[promptId].outputs || {});

              // eslint-disable-next-line no-console
              console.group("[LTX FLF2V] no media output — full diagnostics");
              // eslint-disable-next-line no-console
              console.log("status:", st);
              // eslint-disable-next-line no-console
              console.log("messages (chronological):", messages);
              // eslint-disable-next-line no-console
              console.log("nodes that produced outputs:", executedOutputs);
              // eslint-disable-next-line no-console
              ...
              } else if (execInterrupted) {
                errLine = "Execution interrupted.";
              } else {
                // Common silent-skip causes: missing model file, unknown custom node
                // (validation error excludes a node from execution but never raises
                // execution_error). Surface the executed-node list so the user can
                // spot which branch got skipped.
                errLine += ` Only ran: ${executedOutputs.join(", ") || "(none)"}. VHS_VideoCombine (node 43) was skipped — most likely a missing model/file. Check ComfyUI server log.`;
              }
              setError(errLine);
              log("LTX FLF2V failed", { nodeId: id, nodeType: "fs:ltxFlf", nodeLabel: "LTX 2.3 FLF2V", status: "error", details: errLine });
              setProcessing(false);
              return;
            }
          }
      ...
      log("LTX FLF2V error", { nodeId: id, nodeType: "fs:ltxFlf", nodeLabel: "LTX 2.3 FLF2V", status: "error", details: err.message });
      setProcessing(false);
    }
      ...
      <div className="ltxvideo-node-inner">
        ...
        <div className="ltxvideo-header">
          ...
          <div className="ltxvideo-header-text">
            <span className="ltxvideo-title">LTX 2.3 FLF2V</span>
...
export default memo(LtxFlfNode);
