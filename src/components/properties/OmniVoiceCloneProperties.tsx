...
function OmniVoiceCloneProperties({ nodeId, data }: { nodeId: string; data: any }) {
  ...
  const refText = data.widgetValues?.refText ?? "";
  ...
  return (
    <>
      <div className="props-section">
        <div className="props-section-title">Reference Transcript</div>
        <textarea className="props-textarea" value={refText} rows={2}
          placeholder="Transcript of the reference voice (leave empty to auto-transcribe via Whisper)"
          onChange={(e) => updateWidgetValue(nodeId, "refText", e.target.value)} />
      </div>
      ...
      <div className="props-section">
        <div className="props-section-title">Voice Design (instruct, optional)</div>
      ...
      <details className="props-section props-temp-section">
        ...
        <div className="props-section">
          <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer" }}>
            ...
            <span>Load ASR (Whisper) <span style={{ color: "var(--text-muted)", fontSize: 11 }}>(needed for empty refText)</span></span>
          </label>
        </div>

        <div className="props-section">
          <ul className="settings-hint" style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 0, lineHeight: 1.5, paddingLeft: 16 }}>
            <li><b>Reference Transcript</b> — empty + Load ASR ON ⇒ Whisper transcribes ref automatically. Manual transcript = better fidelity.</li>
            <li><b>Language</b> — output language, can differ from reference voice (cross-lingual cloning).</li>
            <li><b>Voice Design</b> — usually leave empty: ref audio carries the timbre.</li>
            <li><b>Diffusion Steps</b> — 32 default; 16 faster; higher = cleaner.</li>
            <li><b>Guidance (CFG)</b> — 2.0 default; higher = stricter prompt match.</li>
            <li><b>Duration</b> — 0 = auto. Setting overrides speed.</li>
            <li><b>Seed</b> — fix to a number to reproduce; empty = random per run.</li>
            <li><b>Tail clipping?</b> Try Speed=0.9, set Duration manually, disable Postprocess Output, or provide explicit refText. Model is actively updated — <a href="https://github.com/k2-fsa/OmniVoice" target="_blank" rel="noopener noreferrer" style={{ color: "#f472b6" }}>check GitHub</a> for fixes.</li>
          </ul>
        </div>
      </details>
    </>
...
export default OmniVoiceCloneProperties;
